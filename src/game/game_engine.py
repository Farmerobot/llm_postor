...
class GameEngine():
    ...
    def init_game(self) -> None:
        ...
        for player in self.players:
            ...
            if player.is_impostor:
                player.set_tasks([ShortTask("Kill all crewmates", GameLocation.LOC_UNKNOWN)])
                player.history[-1]["role"] = f"Welcome {player.name}\nYou are an impostor. Your goal is to kill or banish all crewmates"
        ...
        while not check_game_over():
            ...
            for player in self.players:
                if player.player_state == PlayerState.ALIVE:
                    ...
                    possible_actions_str = [action.get_input_story() for action in possible_actions]
                    action_int = player.prompt_action("Choose an action", possible_actions_str)
    ...
    def update_game_state(self, actions: list[GameAction]) -> bool:
        ...
        for action in actions:
            ...
            if action.source.player_state != PlayerState.DEAD:
                ...
                if action.action_type == GameActionType.REPORT:
                    self.broadcast_history("report", f"{action.source} reported a dead body")
                    self.broadcast_history("dead_players", f"Dead players found: {', '.join([str(player) for player in self.players if player.player_state == PlayerState.DEAD])}")
                ...
                if action.action_type == GameActionType.KILL and isinstance(action.target, Player):
                    action.target.history[-1]["killed"] = f"You were killed by {action.source}"
                
                action.source.history[-1]["action_result"] = action.do_action() # do action and save result in history

        for action in actions:
            if action.source.player_state != PlayerState.DEAD:
                for player in self.players: # update stories of seen actions and players in room
                    if player.player_state != PlayerState.DEAD and player != action.source:
                        if action.location == player.player_location or action.location == player.prev_location:
                            self.playthrough.append(f"Player {player} saw action {action.get_spectator_story()} when {player} were in {action.location}")
                            if self.DEBUG:
                                print(f"Player {player} saw action {action.get_spectator_story()} when {player} were in {action.location}") # DEBUG
                            seen_actions: list[str] = player.history[-1].get("seen_action", [])
                            seen_actions.append(f"you saw {action.get_spectator_story()} when you were in {action.location}")
        ...
        for player in self.players:
            ...
            player.history[-1]["Current_location"] = f"You are in {player.get_location()}"
            
            self.playthrough.append(f"Player {player} is in {player.get_location()} with {players_in_room}")
            if self.DEBUG:
                print(f"Player {player} is in {player.get_location()} with {players_in_room}")
            if players_in_room:
                player.history[-1]["player_in_room"] = f"Players in room with you: {', '.join([str(player) for player in players_in_room])}"
            else:
    ...
    def get_actions(self, player: Player) -> list[GameAction]:
        ...
        dead_players_in_room = [dead_player for dead_player in self.players if dead_player.player_state == PlayerState.DEAD and dead_player.player_location == player.player_location]
        if dead_players_in_room:
            actions.append(GameAction(GameActionType.REPORT, player, dead_players_in_room))
        # actions for moves
    ...
    def go_to_discussion(self) -> None:
        ...
        for player in self.players:
            ...
            player.history[-1]["discussion"] = "Discussion phase has started. You can discuss and vote who to banish"
        
        discussion_log: list[str] = []
        for round in range(game_consts.NUM_CHATS):
            for player in self.players:
                if player.player_state == PlayerState.ALIVE:
                    player.history[-1]["turns left"] = f"You have {game_consts.NUM_CHATS - round} rounds left to discuss, then you will vote"
    ...
    def go_to_voting(self) -> None:
        dead_players = [player for player in self.players if player.player_state == PlayerState.DEAD_REPORTED]
        votes = {}
        for player in self.players:
            possible_actions = self.get_vote_actions(player)
            possible_voting_actions_str = [action.get_input_story() for action in possible_actions]
            player.history[-1]["found_dead"] = f"Dead players found: {', '.join([str(player) for player in dead_players])}"
            player.history[-1]["voting"] = "Voting phase has started. You can vote who to banish\n"

            if player.player_state == PlayerState.ALIVE:
                action = player.prompt_vote(possible_voting_actions_str)
                votes[player] = possible_actions[action].target
                player.history[-1]["vote"] = f"You voted for {possible_actions[action].target}"
                player.player_location = GameLocation.LOC_CAFETERIA
                self.playthrough.append(f"{player} voted for {possible_actions[action].target}")
                if self.DEBUG:
        ...
        if len(two_most_common) > 1 and two_most_common[0][1] == two_most_common[1][1]:
            ...
            assert isinstance(two_most_common[0][0], Player)  # Ensure that the expression is of type Player
            player_to_banish: Player = two_most_common[0][0]  # Explicitly cast the expression to Player
            if player_to_banish == self.nobody:
                self.broadcast_history("vote", "Nobody was banished!")
            elif player_to_banish.is_impostor:
                self.broadcast_history("vote", f"{player_to_banish} was banished! They were an impostor")
            else:
                self.broadcast_history("vote", f"{player_to_banish} was banished! They were a crewmate")
    ...
    def get_vote_actions(self, player: Player) -> list[GameAction]:
        ...
        for other_player in self.players:
            if other_player != player and other_player.player_state == PlayerState.ALIVE:
    ...
    def check_impostors_win(self) -> bool:
        ...
        impostors_alive = [player for player in self.players if player.is_impostor and player.player_state == PlayerState.ALIVE]
    ...
    def check_win_by_tasks(self) -> bool:
        crewmates_alive = [player for player in self.players if not player.is_impostor and player.player_state == PlayerState.ALIVE]
        if all([task.completed for player in crewmates_alive for task in player.player_tasks]):
            return True
        return False
    ...
    def check_game_over_action_crewmate(self) -> bool:
        ...
        completed_tasks = [task.completed for player in crewmates_alive for task in player.player_tasks]
        if turns_passed >= 100:
            self.playthrough.append(f"Turns passed: {turns_passed}")
            self.playthrough.append(f"Crewmates lose! Too many turns passed! Completed tasks: {completed_tasks}")
