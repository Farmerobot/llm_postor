...
for i, j, _ in get_game_permutations():
    if (i, j) in set_ij:
        continue
    set_ij.add((i, j))
    wins, loses, round_limits, exceptions, game_prices = (
        0,
        0,
        0,
        0,
        [0 for _ in range(n_repetitions)],
    )
    for repetition in range(n_repetitions):
        file_path_pattern = (
        ...
        file_path = ""
        occurrence = filter(lambda x: file_path_pattern in x, file_names)
        if res := list(occurrence):
            file_path = dir + "/" + res[0]
        else:
            continue
    ...
    model_i = ai_models[i].split("/")[-1].replace(".", "-")
    model_j = ai_models[j].split("/")[-1].replace(".", "-")
    print(f"{model_i} vs {model_j}")

output_pd = pd.DataFrame(
    OUTPUT,
    columns=["Impostor", "Crewmate", "Wins", "Loses", "Exceptions", "Round limits"],
)
