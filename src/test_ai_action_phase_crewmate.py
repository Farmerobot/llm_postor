...
# for model_name in ["gpt-4o", "gpt-4-32k"]:
    ...
    for i in range(5, 10):
        ...
        turns_to_complete.append(max([player.turns_passed() for player in game.players]))
        print("End of test")

    print(f"turns_to_complete: {turns_to_complete}")
    print(f"Average turns to complete: {sum(turns_to_complete) / len(turns_to_complete)}")
    print(f"Max turns to complete: {max(turns_to_complete)}")
    print(f"Min turns to complete: {min(turns_to_complete)}")
    with open(f"./game/game_results/action_phase/turns_to_complete_crewmate_action_{model_name}.txt", "w+") as f:
